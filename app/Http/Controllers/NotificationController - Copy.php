...
// FirebaseService::sendNotification($penitip->fcm_token, 'Judul', 'Isi notifikasi');
...
class NotificationController extends Controller
{
    ...
    public function kirimNotifikasi(FirebaseService $fcm)
    {
        $token = 'DEVICE_FCM_TOKEN'; // dari frontend mobile
        $title = 'Judul Notif';
        $body = 'Isi pesan notifikasi';

        $response = $fcm->sendNotification($token, $title, $body);

        return response()->json(['message' => 'Notifikasi terkirim', 'firebase_response' => $response]);
    }
