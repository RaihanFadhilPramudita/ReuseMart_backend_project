...
//             ->join('barang', 'detail_transaksi.ID_BARANG', '=', 'barang.ID_BARANG')
//             ->select('barang.ID_PENITIP', DB::raw('SUM(barang.HARGA) as total_penjualan'))
//             ->whereIn('transaksi.STATUS_TRANSAKSI', ['Selesai', 'Hangus'])
//             ->whereMonth('transaksi.WAKTU_PESAN', $bulan)
//             ->whereYear('transaksi.WAKTU_PESAN', $tahun)
//             ->groupBy('barang.ID_PENITIP')
//             ->orderByDesc('total_penjualan')
...
//         if ($penjualan->isEmpty()) {
//             return response()->json(['message' => 'Tidak ada transaksi bulan lalu.'], 200);
//         }
...
//         // Update penitip top seller
//         $penitip = Penitip::find($topPenitip->ID_PENITIP);
//         $bonus = $topValue * 0.01;
//         $penitip->BADGE = "Top Seller {$namaBulan}";
//         $penitip->SALDO += $bonus;
//         $penitip->save();

//         // Simpan bonusnya ke tabel komisi (boleh dihapus jika tidak perlu)
//         Komisi::create([
//             'JUMLAH_KOMISI_REUSE_MART' => 0,
//             'JUMLAH_KOMISI_HUNTER' => 0,
//             'BONUS_PENITIP' => $bonus,
...
//         return response()->json([
//             'message' => 'Top Seller berhasil diperbarui',
//             'top_seller' => $penitip->NAMA_PENITIP,
//             'total_penjualan' => $topValue,
//             'bonus' => $bonus,
//             'badge' => $penitip->BADGE
...
//     } catch (\Throwable $e) {
//         return response()->json([
//             'message' => 'Gagal hitung top seller',
//             'error' => $e->getMessage()
//         ], 500);
//     }
// }
